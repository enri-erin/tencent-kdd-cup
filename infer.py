...
_FALLBACK_MODEL_CFG = {
    ...
    'user_ns_tokens': 0,
    'item_ns_tokens': 0,
    'use_aligned_user_dense_tokens': True,
    'user_dense_tokens': 1,
    'use_token_type_embeddings': True,
    'use_domain_gating': True,
    'use_representation_enhancements': False,
    'use_coarse_features': True,
    'use_activity_rhythm_features': False,
    'use_target_seq_features': True,
    'use_temporal_sequence_features': True,
    'use_temporal_id_attention_gate': True,
    'use_context_temporal_features': False,
    'use_user_context_temporal_gate': False,
    'use_din_query_pooling': False,
    'use_unimixing_lite': False,
    'unimixing_num_basis': 4,
    'unimixing_global_rank': 4,
    'unimixing_block_size': 0,
    'unimixing_temperature': 1.0,
    'unimixing_sinkhorn_iters': 3,
    'use_unimixing_siamese_norm': False,
    'use_iat_sequence_branch': False,
    'iat_max_tokens': 192,
    'iat_num_layers': 1,
    'iat_ins_emb_dim': 64,
}
...
def resolve_model_cfg(train_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key in _MODEL_CFG_KEYS:
        if key == 'num_time_buckets':
            ...
            continue
        if key == 'use_temporal_sequence_features':
    ...
    if 'token_mixer_type' not in train_config:
        if train_config.get('use_unimixing_lite'):
            cfg['token_mixer_type'] = 'unimixing_lite'
        elif train_config.get('use_unimixing'):
            cfg['token_mixer_type'] = 'unimixing'
    return cfg
...
def build_model(
    ...
    model = PCVRHyFormer(
        ...
        coarse_feat_dim=dataset.coarse_feat_dim,
        coarse_seq_per_domain_dim=dataset.coarse_seq_per_domain_dim,
        coarse_global_seq_dim=dataset.coarse_global_seq_dim,
        coarse_global_static_dim=dataset.coarse_global_static_dim,
        target_seq_feat_dim=dataset.target_seq_feat_dim,
        target_seq_sideinfo_counts=dataset.target_seq_sideinfo_counts,
...
def _sort_batch_by_timestamp(batch: Dict[str, Any]) -> Dict[str, Any]:
    if 'timestamp' not in batch:
        return batch
    order = torch.argsort(batch['timestamp'])
...
def _attach_iat_history(
    ...
    if history_store is None:
        return batch
    batch = _sort_batch_by_timestamp(batch)
    batch.update(
...
def main() -> None:
    ...
    use_iat_sequence_branch = bool(train_config.get('use_iat_sequence_branch', False))
    if use_iat_sequence_branch:
        num_workers = 1 if num_workers > 0 else 0
        logging.info(
            "IAT enabled during inference: forcing ordered history replay "
    ...
    test_dataset = PCVRParquetDataset(
        ...
        buffer_batches=0,
        is_training=False,
        use_coarse_features=bool(train_config.get('use_coarse_features', True)),
        use_activity_rhythm_features=bool(
            train_config.get('use_activity_rhythm_features', False)
        ),
        use_target_seq_features=bool(train_config.get('use_target_seq_features', True)),
        use_temporal_sequence_features=bool(
            train_config.get('use_temporal_sequence_features', True)
        ),
        use_context_temporal_features=(
            bool(train_config.get('use_context_temporal_features', False))
            or bool(train_config.get('use_user_context_temporal_gate', False))
        ),
        temporal_utc_offset=train_config.get(
            'temporal_utc_offset',
            DEFAULT_TEMPORAL_UTC_OFFSET,
        ),
    ...
    all_probs = []
    all_user_ids = []
    iat_history_store = None
    if use_iat_sequence_branch:
        iat_history_store = IATHistoryStore(
            history_len=int(model_cfg.get('iat_max_tokens', 0)),
            ins_emb_dim=int(model_cfg.get('iat_ins_emb_dim', 0)),
            bucket_boundaries=BUCKET_BOUNDARIES,
            task_hash_size=int(model_cfg.get('iat_task_hash_size', 65536)),
            device=device,
        )
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(test_loader):
            batch = _attach_iat_history(batch, iat_history_store)
            model_input = _batch_to_model_input(batch, device)
            user_ids = batch.get('user_id', [])

            if use_iat_sequence_branch and hasattr(model, 'forward_with_aux'):
                outputs = model.forward_with_aux(model_input)
                logits = outputs['logits']
                current_ins_emb = outputs.get('current_ins_emb')
                if iat_history_store is not None and current_ins_emb is not None:
                    iat_history_store.update(
                        user_ids=user_ids,
                        timestamps=batch['timestamp'],

...
DEFAULT_TEMPORAL_UTC_OFFSET = 'UTC+8'
...
ACTIVITY_RHYTHM_SEQ_FEATS_PER_DOMAIN = 4
ACTIVITY_RHYTHM_GLOBAL_SEQ_FEATS = 4
...
COARSE_RECENT_1D_SECONDS = 86400
COARSE_RECENT_30D_SECONDS = 2592000
...
def _date_to_day_index(day_str: str) -> int:
    cur = date.fromisoformat(day_str)
    return (cur - date(1970, 1, 1)).days


def _expand_day_range(start_day: str, end_day: str) -> List[int]:
    start = date.fromisoformat(start_day)
    end = date.fromisoformat(end_day)
    days: List[int] = []
    cur = start
    while cur <= end:
        days.append((cur - date(1970, 1, 1)).days)
        cur += timedelta(days=1)
    return days


def _build_holiday_day_indices() -> Tuple[np.ndarray, np.ndarray]:
    holiday_days: List[int] = []
    makeup_workdays: List[int] = []

    holiday_days.extend(_expand_day_range('2026-01-01', '2026-01-03'))
    makeup_workdays.append(_date_to_day_index('2026-01-04'))
    holiday_days.extend(_expand_day_range('2026-02-15', '2026-02-23'))
    makeup_workdays.extend([
        _date_to_day_index('2026-02-14'),
        _date_to_day_index('2026-02-28'),
    ])
    holiday_days.extend(_expand_day_range('2026-04-04', '2026-04-06'))
    holiday_days.extend(_expand_day_range('2026-05-01', '2026-05-05'))
    makeup_workdays.append(_date_to_day_index('2026-05-09'))
    holiday_days.extend(_expand_day_range('2026-06-19', '2026-06-21'))
    holiday_days.extend(_expand_day_range('2026-09-25', '2026-09-27'))
    holiday_days.extend(_expand_day_range('2026-10-01', '2026-10-07'))
    makeup_workdays.extend([
        _date_to_day_index('2026-09-20'),
        _date_to_day_index('2026-10-10'),
    ])

    holiday_days.append(_date_to_day_index('2025-01-01'))
    holiday_days.extend(_expand_day_range('2025-01-28', '2025-02-04'))
    makeup_workdays.extend([
        _date_to_day_index('2025-01-26'),
        _date_to_day_index('2025-02-08'),
    ])
    holiday_days.extend(_expand_day_range('2025-04-04', '2025-04-06'))
    holiday_days.extend(_expand_day_range('2025-05-01', '2025-05-05'))
    makeup_workdays.append(_date_to_day_index('2025-04-27'))
    holiday_days.extend(_expand_day_range('2025-05-31', '2025-06-02'))
    holiday_days.extend(_expand_day_range('2025-10-01', '2025-10-08'))
    makeup_workdays.extend([
        _date_to_day_index('2025-09-28'),
        _date_to_day_index('2025-10-11'),
    ])

    holiday_days.append(_date_to_day_index('2024-01-01'))
    holiday_days.extend(_expand_day_range('2024-02-10', '2024-02-17'))
    makeup_workdays.extend([
        _date_to_day_index('2024-02-04'),
        _date_to_day_index('2024-02-18'),
    ])
    holiday_days.extend(_expand_day_range('2024-04-04', '2024-04-06'))
    makeup_workdays.append(_date_to_day_index('2024-04-07'))
    holiday_days.extend(_expand_day_range('2024-05-01', '2024-05-05'))
    makeup_workdays.extend([
        _date_to_day_index('2024-04-28'),
        _date_to_day_index('2024-05-11'),
    ])
    holiday_days.append(_date_to_day_index('2024-06-10'))
    holiday_days.extend(_expand_day_range('2024-09-15', '2024-09-17'))
    makeup_workdays.append(_date_to_day_index('2024-09-14'))
    holiday_days.extend(_expand_day_range('2024-10-01', '2024-10-07'))
    makeup_workdays.extend([
        _date_to_day_index('2024-09-29'),
        _date_to_day_index('2024-10-12'),
    ])

    return (
        np.asarray(sorted(set(holiday_days)), dtype=np.int64),
        np.asarray(sorted(set(makeup_workdays)), dtype=np.int64),
    )


HOLIDAY_DAY_INDICES, MAKEUP_WORKDAY_DAY_INDICES = _build_holiday_day_indices()


def parse_utc_offset_to_seconds(offset: Union[str, int, float]) -> int:
    """Parse a UTC offset spec into seconds."""
    if isinstance(offset, (int, float)):
        return int(round(float(offset) * 3600.0))

    raw = str(offset).strip()
    if not raw:
        raise ValueError("temporal_utc_offset must not be empty")
    ...
    if re.fullmatch(r'[+-]?\d+(\.\d+)?', upper):
        return int(round(float(upper) * 3600.0))

    match = re.fullmatch(r'([+-]?)(\d{1,2}):(\d{2})', upper)
    if match is not None:
        sign_str, hour_str, minute_str = match.groups()
    ...
    raise ValueError(
        f"Unsupported temporal_utc_offset={offset!r}. "
...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        temporal_utc_offset: Union[str, int, float] = DEFAULT_TEMPORAL_UTC_OFFSET,
        ...
        Args:
            ...
            use_activity_rhythm_features: whether to append extra user history
                cadence/activity features to the coarse branch.
            ...
            use_context_temporal_features: whether to expose absolute-time
                features for the current sample timestamp as model inputs.
            temporal_utc_offset: UTC offset used when deriving absolute-time
                features such as hour / weekday / rest-day / holiday.
        ...
        self.use_activity_rhythm_features = (
            bool(use_activity_rhythm_features) and self.use_coarse_features
        )
        self.use_target_seq_features = use_target_seq_features
        self.use_temporal_sequence_features = use_temporal_sequence_features
        self.use_context_temporal_features = use_context_temporal_features
        self.temporal_utc_offset = temporal_utc_offset
        self.seq_local_time_offset_seconds = parse_utc_offset_to_seconds(
            temporal_utc_offset
        )
        ...
        self._buf_context_temporal_ids = np.zeros((B, CONTEXT_TEMPORAL_ID_DIM), dtype=np.int64)
        self._buf_context_temporal_dense = np.zeros((B, CONTEXT_TEMPORAL_DENSE_DIM), dtype=np.float32)
        self._buf_seq = {}
        self._buf_seq_tb = {}
        ...
        rhythm_seq_dim = (
            ACTIVITY_RHYTHM_SEQ_FEATS_PER_DOMAIN
            if self.use_activity_rhythm_features else 0
        )
        rhythm_global_dim = (
            ACTIVITY_RHYTHM_GLOBAL_SEQ_FEATS
            if self.use_activity_rhythm_features else 0
        )
        self.coarse_seq_per_domain_dim = (
            COARSE_SEQ_FEATS_PER_DOMAIN + rhythm_seq_dim
            if self.use_coarse_features else 0
        )
        self.coarse_global_seq_dim = (
            COARSE_GLOBAL_SEQ_FEATS + rhythm_global_dim
            if self.use_coarse_features else 0
        )
    ...
    def _build_temporal_features(
        ...
        Discrete ids layout (11 dims):
        0: hour_id
        1: weekday_id
        2: holiday-adjusted rest-day id
        3: holiday id
        4: inter-event gap bucket id
        5-7: request/session/visit position ids
        8-10: request/session/visit boundary ids
        ...
        local_ts = valid_ts + self.seq_local_time_offset_seconds
        day_index = local_ts // 86400
        hour = ((local_ts % 86400) // 3600).astype(np.float32)
        weekday = ((day_index + 3) % 7).astype(np.float32)
        is_makeup_workday = np.isin(day_index, MAKEUP_WORKDAY_DAY_INDICES)
        is_holiday = np.isin(day_index, HOLIDAY_DAY_INDICES)
        is_rest_day = ((weekday >= 5.0) & ~is_makeup_workday) | is_holiday

        hour_ids = hour.astype(np.int64) + 1
        weekday_ids = weekday.astype(np.int64) + 1
        rest_day_ids = is_rest_day.astype(np.int64) + 1
        holiday_ids = is_holiday.astype(np.int64) + 1
        temporal_ids[valid_mask, 0] = hour_ids
        temporal_ids[valid_mask, 1] = weekday_ids
        temporal_ids[valid_mask, 2] = rest_day_ids
        temporal_ids[valid_mask, 3] = holiday_ids
        ...
        if nonzero_gap.any():
            ...
            temporal_ids[nonzero_gap, 4] = gap_bucket.astype(np.int64)
        ...
        for pos, ts_val in enumerate(raw_ts.tolist()):
            ...
            temporal_ids[pos, 5] = min(request_pos + 1, int(SEQ_SESSION_POS_NORM_MAX))
            temporal_ids[pos, 6] = min(session_pos + 1, int(SEQ_SESSION_POS_NORM_MAX))
            temporal_ids[pos, 7] = min(visit_pos + 1, int(SEQ_SESSION_POS_NORM_MAX))
            temporal_ids[pos, 8] = int(new_request) + 1
            temporal_ids[pos, 9] = int(new_session) + 1
            temporal_ids[pos, 10] = int(new_visit) + 1
            prev_ts = ts_val
    ...
    def _build_context_temporal_features(
        ...
        """Build absolute-time features for the current sample timestamp."""
        raw_ts = np.asarray(timestamps, dtype=np.int64)
        temporal_ids = np.zeros((raw_ts.shape[0], CONTEXT_TEMPORAL_ID_DIM), dtype=np.int64)
        feats = np.zeros((raw_ts.shape[0], CONTEXT_TEMPORAL_DENSE_DIM), dtype=np.float32)
        valid_mask = raw_ts > 0
        if not valid_mask.any():
            return temporal_ids, feats
        ...
        temporal_ids[valid_mask, 0] = hour.astype(np.int64) + 1
        temporal_ids[valid_mask, 1] = weekday.astype(np.int64) + 1
        temporal_ids[valid_mask, 2] = is_rest_day.astype(np.int64) + 1
        temporal_ids[valid_mask, 3] = is_holiday.astype(np.int64) + 1
    ...
    def _compute_target_seq_row_features(
        ...
        age_seconds: "npt.NDArray[np.int64]",
    ) -> "npt.NDArray[np.float32]":
        """Compute 8 multi-window target-seq heat features.

        Uses exact timestamp-delta multi-window statistics to capture how often the
        ...
        information is used.  When exact timestamps are unavailable a
        ...
        # Align exact age seconds with seq_vals.  Missing timestamp positions
        # stay negative so a true zero-second age remains valid.
        time_seconds = np.asarray(age_seconds, dtype=np.int64)
        if time_seconds.shape[0] < seq_vals.shape[0]:
            padded = np.full(seq_vals.shape[0], -1, dtype=np.int64)
            padded[:time_seconds.shape[0]] = time_seconds
            time_seconds = padded
        elif time_seconds.shape[0] > seq_vals.shape[0]:
            time_seconds = time_seconds[:seq_vals.shape[0]]

        valid_time = time_seconds >= 0
        use_position_fallback = True
        if valid_time.any():
            time_seconds = np.maximum(time_seconds, 0)
            use_position_fallback = False

        if use_position_fallback:
            # Assume roughly 1 hour per position when no real timestamps exist.
            time_seconds = (
        ...
        for name, seconds in window_seconds:
            in_window = active & (time_seconds <= seconds)
            active_count = max(int(in_window.sum()), 1)
            hit_count = int((effective & in_window).sum())
            heat[name] = float(
                np.log1p(float(hit_count)) / np.log1p(float(active_count))
            )
        ...
        feats[4] = 1.0 - heat['1d']
        feats[5] = max(heat['30d'] - heat['1d'], 0.0)
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        if self.use_context_temporal_features:
            context_ids = self._buf_context_temporal_ids[:B]
            context_dense = self._buf_context_temporal_dense[:B]
            context_ids[:], context_dense[:] = self._build_context_temporal_features(timestamps)
            result['context_temporal_ids'] = torch.from_numpy(context_ids.copy())
            result['context_temporal_dense'] = torch.from_numpy(context_dense.copy())
        ...
        for domain_idx, domain in enumerate(self.seq_domains):
            ...
            raw_age_seconds = [] if need_target else None
            if need_coarse:
                ...
                recent_1d_ratio = np.zeros(B, dtype=np.float32)
                recent_30d_ratio = np.zeros(B, dtype=np.float32)
                last_age_norm = np.ones(B, dtype=np.float32)
                avg_gap_norm = np.zeros(B, dtype=np.float32)
            for i in range(B):
                ...
                if need_target:
                    raw_age = np.full(domain_raw_len, -1, dtype=np.int64)
                    if ts_offs is not None and ts_vals is not None:
                        s = int(ts_offs[i])
                        e = int(ts_offs[i + 1])
                        raw_ts = np.asarray(ts_vals[s:e], dtype=np.int64)
                        valid_ts = raw_ts > 0
                        exact_age = np.full(raw_ts.shape[0], -1, dtype=np.int64)
                        exact_age[valid_ts] = np.maximum(
                            int(timestamps[i]) - raw_ts[valid_ts],
                            0,
                        )
                        use_len = min(domain_raw_len, exact_age.shape[0])
                        raw_age[:use_len] = exact_age[:use_len]
                    raw_age_seconds.append(raw_age)
                ...
                recent_1d_ratio[i] = float((time_diff <= COARSE_RECENT_1D_SECONDS).mean())
                recent_7d_ratio[i] = float((time_diff <= COARSE_RECENT_7D_SECONDS).mean())
                recent_30d_ratio[i] = float((time_diff <= COARSE_RECENT_30D_SECONDS).mean())
                freshness_score[i] = float(
                    np.exp(-time_diff.astype(np.float32) / COARSE_FRESHNESS_TAU_SECONDS).mean()
                )
                last_age_norm[i] = float(
                    np.log1p(time_diff.min()) / np.log1p(SEQ_TIME_NORM_MAX_SECONDS)
                )
                if valid_ts.size > 1:
                    ...
                    gaps = gaps[gaps > 0]
                    if gaps.size > 0:
                        avg_gap_norm[i] = float(
                            np.log1p(gaps.astype(np.float32).mean())
                            / np.log1p(SEQ_TIME_NORM_MAX_SECONDS)
                        )
            ...
            if need_coarse:
                ...
                if self.use_activity_rhythm_features:
                    rhythm_base = (
                        len(self.seq_domains) * COARSE_SEQ_FEATS_PER_DOMAIN
                        + domain_idx * ACTIVITY_RHYTHM_SEQ_FEATS_PER_DOMAIN
                    )
                    coarse[:, rhythm_base + 0] = recent_1d_ratio
                    coarse[:, rhythm_base + 1] = recent_30d_ratio
                    coarse[:, rhythm_base + 2] = last_age_norm
                    coarse[:, rhythm_base + 3] = avg_gap_norm
        ...
        if need_coarse and active_step_counts:
            ...
            global_base = len(self.seq_domains) * self.coarse_seq_per_domain_dim
            ...
            if self.use_activity_rhythm_features:
                active_total = active_steps_arr.sum(axis=1)
                active_domain_count = np.maximum(
                    (active_steps_arr > 0).sum(axis=1),
                    1,
                ).astype(np.float32)
                coarse[:, global_base + 4] = np.log1p(active_total) / np.log1p(total_max_len)
                coarse[:, global_base + 5] = active_total / np.maximum(raw_total_lengths, 1.0)
                coarse[:, global_base + 6] = recent_domain_arr.sum(axis=1) / active_domain_count
                coarse[:, global_base + 7] = np.clip(raw_total_lengths / total_max_len, 0.0, 1.0)
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    train_dataset = PCVRParquetDataset(
        ...
        use_activity_rhythm_features=use_activity_rhythm_features,
        use_target_seq_features=use_target_seq_features,
        use_temporal_sequence_features=use_temporal_sequence_features,
        use_context_temporal_features=use_context_temporal_features,
        temporal_utc_offset=temporal_utc_offset,

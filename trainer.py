...
from dataset import (
    BUCKET_BOUNDARIES,
    CONTEXT_TEMPORAL_DENSE_DIM,
    CONTEXT_TEMPORAL_ID_DIM,
    SEQ_TEMPORAL_DENSE_DIM,
    SEQ_TEMPORAL_ID_DIM,
)
from utils import sigmoid_focal_loss, EarlyStopping, IATHistoryStore
...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        use_amp: bool = False,
        iat_source_loss_weight: float = 1.0,
        unimixing_temperature_end: float = 1.0,
        unimixing_temperature_anneal_steps: int = 0,
    ) -> None:
        ...
        self.use_iat_sequence_branch: bool = bool(
            getattr(model, 'use_iat_sequence_branch', False)
        )
        self.iat_source_loss_weight: float = iat_source_loss_weight
        self.unimixing_temperature_start = (
            float(model.get_unimixing_temperature())
            if hasattr(model, 'get_unimixing_temperature')
            and model.get_unimixing_temperature() is not None
            else None
        )
        self.unimixing_temperature_end = float(unimixing_temperature_end)
        self.unimixing_temperature_anneal_steps = max(0, int(unimixing_temperature_anneal_steps))
        if self.use_iat_sequence_branch:
            self.iat_history_store: Optional[IATHistoryStore] = IATHistoryStore(
                history_len=int(getattr(model, 'iat_history_len', 0)),
                ins_emb_dim=int(getattr(model, 'iat_ins_emb_dim', 0)),
                bucket_boundaries=BUCKET_BOUNDARIES,
                task_hash_size=int(getattr(model, 'iat_task_hash_size', 65536)),
                device=self.device,
            )
        else:
            self.iat_history_store = None

        logging.info(f"PCVRHyFormerRankingTrainer loss_type={loss_type}, "
                     f"focal_alpha={focal_alpha}, focal_gamma={focal_gamma}, "
                     f"reinit_sparse_after_epoch={reinit_sparse_after_epoch}, "
                     f"use_amp={self.use_amp}, amp_dtype={self.amp_dtype}")
        if self.unimixing_temperature_start is not None:
            logging.info(
                "UniMixing temperature schedule: start=%.4f end=%.4f anneal_steps=%d",
                self.unimixing_temperature_start,
                self.unimixing_temperature_end,
                self.unimixing_temperature_anneal_steps,
            )

    def _maybe_update_unimixing_temperature(self, step_idx: int) -> None:
        if (
            self.unimixing_temperature_start is None
            or not hasattr(self.model, 'set_unimixing_temperature')
        ):
            return
        if self.unimixing_temperature_anneal_steps <= 0:
            temperature = self.unimixing_temperature_start
        else:
            progress = min(step_idx, self.unimixing_temperature_anneal_steps)
            ratio = progress / self.unimixing_temperature_anneal_steps
            temperature = (
                self.unimixing_temperature_start
                + (self.unimixing_temperature_end - self.unimixing_temperature_start) * ratio
            )
        self.model.set_unimixing_temperature(temperature)

    def _sort_batch_by_timestamp(self, batch: Dict[str, Any]) -> Dict[str, Any]:
        if 'timestamp' not in batch:
            return batch
        timestamps = batch['timestamp']
        if timestamps.numel() <= 1 or bool(torch.all(timestamps[:-1] <= timestamps[1:])):
            return batch
        order = torch.argsort(timestamps)
        batch_size = int(order.shape[0])
        order_list = order.tolist()
        sorted_batch: Dict[str, Any] = {}
        for key, value in batch.items():
            if isinstance(value, torch.Tensor) and value.shape[:1] == (batch_size,):
                sorted_batch[key] = value.index_select(0, order)
            elif isinstance(value, list) and len(value) == batch_size:
                sorted_batch[key] = [value[idx] for idx in order_list]
            else:
                sorted_batch[key] = value
        return sorted_batch

    def _attach_iat_history(
        self,
        batch: Dict[str, Any],
        history_store: Optional[IATHistoryStore],
    ) -> Dict[str, Any]:
        if not self.use_iat_sequence_branch or history_store is None:
            return batch
        batch = self._sort_batch_by_timestamp(batch)
        batch.update(
            history_store.build_batch(
    ...
    def _update_iat_history_store(
        self,
        history_store: Optional[IATHistoryStore],
        batch: Dict[str, Any],
        current_ins_emb: Optional[torch.Tensor],
    ) -> None:
        if history_store is None or current_ins_emb is None:
            return
        history_store.update(
            ...
            task_ids=batch.get('item_id'),
        )

    def _manual_unscale_sparse_grads(self, scale: float) -> None:
        if self.sparse_optimizer is None or scale == 0.0:
            return
        inv_scale = 1.0 / scale
    ...
    def train(self) -> None:
        ...
        for epoch in range(1, self.num_epochs + 1):
            if self.iat_history_store is not None:
                self.iat_history_store.clear()
            train_pbar = tqdm(enumerate(self.train_loader), total=len(self.train_loader),
                              dynamic_ncols=True)
            loss_sum = 0.0

            for step, batch in train_pbar:
                self._maybe_update_unimixing_temperature(total_step + 1)
    ...
    def _make_model_input(self, device_batch: Dict[str, Any]) -> ModelInput:
        ...
        seq_temporal_ids: Dict[str, torch.Tensor] = {}
        seq_temporal_dense: Dict[str, torch.Tensor] = {}
        for domain in seq_domains:
            ...
            seq_temporal_ids[domain] = device_batch.get(
                f'{domain}_temporal_ids',
                torch.zeros(
                    B,
                    L,
                    SEQ_TEMPORAL_ID_DIM,
            ...
            seq_temporal_dense[domain] = device_batch.get(
                f'{domain}_temporal_dense',
                torch.zeros(
                    B,
                    L,
                    SEQ_TEMPORAL_DENSE_DIM,
        ...
        return ModelInput(
            ...
            target_seq_feats=device_batch.get(
                'target_seq_feats',
                torch.zeros(
                    device_batch['user_int_feats'].shape[0],
                    0,
            ...
            seq_temporal_ids=seq_temporal_ids,
            seq_temporal_dense=seq_temporal_dense,
            context_temporal_ids=device_batch.get(
                'context_temporal_ids',
                torch.zeros(
                    device_batch['user_int_feats'].shape[0],
                    CONTEXT_TEMPORAL_ID_DIM,
                    dtype=torch.long,
            ...
            context_temporal_dense=device_batch.get(
                'context_temporal_dense',
                torch.zeros(
                    device_batch['user_int_feats'].shape[0],
                    CONTEXT_TEMPORAL_DENSE_DIM,
                    dtype=torch.float32,
            ...
            iat_hist_ins_emb=device_batch.get(
                'iat_hist_ins_emb',
                torch.zeros(
                    device_batch['user_int_feats'].shape[0],
                    0,
                    getattr(self.model, 'iat_ins_emb_dim', 0),
                    dtype=torch.float32,
                    device=self.device,
                ),
            ),
            iat_hist_labels=device_batch.get(
                'iat_hist_labels',
                torch.zeros(
                    device_batch['user_int_feats'].shape[0],
                    0,
            ...
            iat_hist_time_buckets=device_batch.get(
                'iat_hist_time_buckets',
                torch.zeros(
                    device_batch['user_int_feats'].shape[0],
                    0,
                    dtype=torch.long,
            ...
            iat_hist_task_ids=device_batch.get(
                'iat_hist_task_ids',
                torch.zeros(
                    device_batch['user_int_feats'].shape[0],
                    0,
            ...
            iat_hist_mask=device_batch.get(
                'iat_hist_mask',
                torch.ones(
                    device_batch['user_int_feats'].shape[0],
                    0,
    ...
    def _train_step(self, batch: Dict[str, Any]) -> float:
        ...
        batch = self._attach_iat_history(batch, self.iat_history_store)
        ...
        with torch.amp.autocast(
            ...
            if self.use_iat_sequence_branch and hasattr(self.model, 'forward_with_aux'):
                model_outputs = self.model.forward_with_aux(model_input)
                logits = model_outputs['logits']
                source_logits = model_outputs.get('source_logits')
                current_ins_emb = model_outputs.get('current_ins_emb')
            else:
                logits = self.model(model_input)  # (B, 1)
                source_logits = None
                current_ins_emb = None
            ...
            if source_logits is not None:
                ...
                if self.loss_type == 'focal':
                    source_loss = sigmoid_focal_loss(
                        source_logits, label, alpha=self.focal_alpha, gamma=self.focal_gamma
                    )
                else:
                    source_loss = F.binary_cross_entropy_with_logits(source_logits, label)
                loss = loss + self.iat_source_loss_weight * source_loss

        if self.use_amp:
            curr_scale = float(self.grad_scaler.get_scale())
            self.grad_scaler.scale(loss).backward()
            # Only the dense optimizer is managed by GradScaler. Sparse
            # gradients are manually unscaled because torch.amp does not
            # reliably support sparse Adagrad unscale/step in all versions.
            self.grad_scaler.unscale_(self.dense_optimizer)
            self._manual_unscale_sparse_grads(curr_scale)
        else:
            loss.backward()
        ...
        if self.dense_params:
            torch.nn.utils.clip_grad_norm_(self.dense_params, max_norm=1.0, foreach=False)
        ...
        if self.use_amp:
            ...
            if self.sparse_optimizer is not None:
                self.sparse_optimizer.step()
        self._update_iat_history_store(self.iat_history_store, batch, current_ins_emb)
    ...
    def evaluate(self, epoch: Optional[int] = None) -> Tuple[float, float]:
        ...
        eval_history_store = self.iat_history_store.clone() if self.iat_history_store is not None else None
        ...
        with torch.no_grad():
            for step, batch in pbar:
                batch = self._attach_iat_history(batch, eval_history_store)
                logits, labels, current_ins_emb = self._evaluate_step(batch)
                all_logits_list.append(logits.detach().cpu())
                all_labels_list.append(labels.detach().cpu())
                self._update_iat_history_store(eval_history_store, batch, current_ins_emb)
    ...
    def _evaluate_step(
        ...
        with torch.amp.autocast(
            ...
            if self.use_iat_sequence_branch and hasattr(self.model, 'forward_with_aux'):
                outputs = self.model.forward_with_aux(model_input)
                logits = outputs['logits']
                current_ins_emb = outputs.get('current_ins_emb')
            else:
                logits, _ = self.model.predict(model_input)  # (B, 1), (B, D)
                current_ins_emb = None
            logits = logits.squeeze(-1)  # (B,)

        return logits, label, current_ins_emb
